...
class CNNBlock(nn.Module):
    ...
    def __init__(self, n_channels, kernel_size=3):
        ...
        self.conv1 = nn.Conv2d(
            2 * n_channels, n_channels, kernel_size, 1, kernel_size // 2
        )
...
class AffineCoupling(nn.Module):
    ...
    def __init__(self, m: nn.Module, modify_x2=True, chw=(1, 28, 28)):
        ...
        self.mask = torch.tensor(
            [[(j + k) % 2 == 0 for k in range(w)] for j in range(h)]
        )
...
def training_loop(model, epochs, lr, loader, device, dir):
    ...
    to_bpd = np.log2(np.exp(1)) / (
        28 * 28 * 1
    )  # Constant that normalizes w.r.t. input shape
    ...
    for epoch in tqdm(range(epochs), desc="Training progress", colour="#00ff00"):
        epoch_loss = 0.0
        for batch in tqdm(
            loader, leave=False, desc=f"Epoch {epoch + 1}/{epochs}", colour="#005500"
        ):
...
def main():
    ...
    parser.add_argument(
        "--store_dir", type=str, default=os.getcwd(), help="Store directory"
    )
