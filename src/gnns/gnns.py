...
def parse_args():
    ...
    parser.add_argument(
        f"--image_size",
        type=int,
        help="Size to which reshape CIFAR images. Default is 14 (196 nodes).",
        default=14,
    )
    ...
    parser.add_argument(
        f"--type",
        type=str,
        help="Type of the network used. Default is 'attn'",
    ...
    parser.add_argument(
        ...
        choices=list(AGGREGATION_FUNCTIONS.keys()),
        default="avg",
    )
    parser.add_argument(
        f"--aggregation_out",
        type=str,
        help="Aggregation function for graph classification",
        choices=list(AGGREGATION_FUNCTIONS.keys()),
        default="avg",
    )
    parser.add_argument(
        f"--n_layers", type=int, help="Number of layers of the GNNs", default=8
    )

    # Training arguments
    parser.add_argument(
        f"--epochs", type=int, help="Training epochs. Default is 10.", default=10
    )
    parser.add_argument(
        f"--lr", type=float, help="Learning rate. Default is 1e-3.", default=0.001
    )
    parser.add_argument(
        f"--batch_size",
        type=int,
        help="Batch size used for training. Default is 64.",
        default=64,
    )
    parser.add_argument(
        f"--checkpoint",
        type=str,
        help="Path to model checkpoints. Default is 'gnn.pt'.",
        default="gnn.pt",
    )
...
class GraphNeuralNetwork(nn.Module):
    ...
    _NET_TYPE_TO_LAYER = {"attn": GraphAttentionLayer, "conv": GraphConvLayer}
    ...
    def __init__(self, net_type, n_layers, n, d_in, d_hidden, d_out, aggr, aggr_out):
        ...
        assert (
            net_type in NETWORK_TYPES
        ), f"ERROR: GNN type {net_type} not supported. Pick one of {NETWORK_TYPES}"
        ...
        self.layers = nn.ModuleList(
            [
                self._NET_TYPE_TO_LAYER[net_type](
                    n, d_hidden, AGGREGATION_FUNCTIONS[aggr]
                )
        ...
        self.out_mlp = nn.Sequential(
            nn.Linear(d_hidden, d_hidden), nn.ReLU(), nn.Linear(d_hidden, d_out)
        )
...
def main():
    ...
    train_set = MNIST("./../datasets", train=True, transform=transform, download=True)
    test_set = MNIST("./../datasets", train=False, transform=transform, download=True)
    ...
    model = GraphNeuralNetwork(
        ...
        aggr_out=args["aggregation_out"],
    )

    # Training loop
    n_epochs = args["epochs"]
    checkpoint = (
        args["checkpoint"] if args["checkpoint"] else f"({args['type']}_gnn).pt"
    )
